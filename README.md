...
## 顶层模块
- **`systemtop.v`**：该 systemtop 顶层把 DE1/同类 FPGA 板上的多个外设与逻辑整合成一个“智能十字路口演示系统”：在 VGA 640×480@60Hz 上实时渲染十字路口交通灯场景（含灯色、相位倒计时、车辆与行人动画、违规爆炸特效等），交通灯控制核心在 50MHz 时钟域下依据 模式选择（固定/感应/夜间/锁定）、车辆请求以及行人请求输出 NS/EW 灯色与相位剩余时间；同时，摄像头经 MIPI Bridge→SDRAM 缓存→RAW2RGB→缩放到 200×150 形成画中画 PIP，并由 pip_cam_200x150 直接提供系统所用 25MHz 像素时钟，最终在像素时钟域将PIP（右下角有效且 I2C 初始化完成时）与基础场景进行叠加输出到 VGA；整个系统还带有按键硬复位与键盘软复位管理、数码管显示相位/倒计时/模式、以及 LED 状态指示用于调试。
